...
public class CustomizedResponseEntityExceptionHelper extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ErrorDetails> handlePersonNotFoundException(PersonNotFoundException ex,
			WebRequest request) {
		LOGGER.debug("entering handlePersonNotFoundException()");
//		LOGGER.error(ex.getMessage());
		ErrorDetails errorDetails = new ErrorDetails(new Date(), ex.getMessage(), request.getDescription(false));
		
		ResponseEntity<ErrorDetails> responseEntity = new ResponseEntity<>(errorDetails, HttpStatus.INTERNAL_SERVER_ERROR);
		LOGGER.error("returning " + responseEntity);
